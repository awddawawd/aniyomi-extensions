...
class VoirAnimeVF : ParsedAnimeHttpSource() {
    ...
    private fun popularAnimeRequest(page: Int, filterList: AnimeFilterList): Request {
        ...
        val url = if (selectedGenre != null) {
            ...
            if (page == 1) "$baseUrl/anime-genre/$slug/?filter=dubbed"
            else "$baseUrl/anime-genre/$slug/page/$page/?filter=dubbed"
        } else {
            // Modification vers VF (dubbed)
            if (page == 1) "$baseUrl/?filter=dubbed"
            else "$baseUrl/page/$page/?filter=dubbed"
        }
